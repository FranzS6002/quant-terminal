...
sat_tickers = {
    ...
    "REUSE.MI": "Circular Economy", "SNSR.MI": "IoT", "XUFN.DE": "USA Financials",
    "XNGI.DE": "Next Gen", "HYCN.DE": "Global X Hydrogen", "BATE.DE": "Battery Value Chain", 
    "AMEC.DE": "MSCI Smart Cities",
    
    # --- 🔥 NEUZUGÄNGE: REBALANCING 06/2026 🔥 ---
    "DBX5.DE": "MSCI Taiwan",
    "LKOR.DE": "MSCI Korea",
    "LI7U.DE": "Lithium & Battery Tech",
    "AIFS.DE": "AI-Infrastruktur",
    "USPY.DE": "L&G CyberSecurity",
    "SLVR.DE": "Glob.X Silver Miners",
    "KWBE.DE": "Kranesh.China Internet"
}
...
HOLD_RANK_BUFFER = st.sidebar.number_input("Hysterese-Puffer (+X)", min_value=0, max_value=10, value=def_hyst)
HOLD_RANK = BUY_RANK + HOLD_RANK_BUFFER

REBAL_INTERVAL = st.sidebar.number_input("Rebalancing-Intervall (Tage)", min_value=5, max_value=60, value=def_rebal)

st.sidebar.markdown("---")
st.sidebar.header("🧠 Smart-Scoring (Neu)")
if app_mode == "🚀 Satelliten (Offensive)":
    st.sidebar.caption("Gewichtung des Momentum-Scores")
    w_1m = st.sidebar.slider("1 Monat (%)", 0, 100, 40)
    w_3m = st.sidebar.slider("3 Monate (%)", 0, 100, 10)
    w_6m = st.sidebar.slider("6 Monate (%)", 0, 100, 0)
    w_12m = st.sidebar.slider("12 Monate (%)", 0, 100, 50)
    
    # Normalisierung falls Summe != 100
    total_w = w_1m + w_3m + w_6m + w_12m
    if total_w == 0: total_w = 1 # Division durch Null verhindern
    w_1m, w_3m, w_6m, w_12m = w_1m/total_w, w_3m/total_w, w_6m/total_w, w_12m/total_w
    
    st.sidebar.caption("Preis-Glättung (Anti-Whipsaw)")
    smooth_days = st.sidebar.slider("Glättung (Tage)", min_value=1, max_value=20, value=3, help="1 = Keine Glättung. Höhere Werte ignorieren wilde Tagesausschläge und stabilisieren das Ranking.")
else:
    w_1m, w_3m, w_6m, w_12m = 0.25, 0.25, 0.25, 0.25
    smooth_days = 1
...
if app_mode == "🚀 Satelliten (Offensive)":
    TRAILING_STOP_PCT = st.sidebar.number_input("Trailing Stop-Loss (%)", min_value=5.0, max_value=50.0, value=20.0, step=0.5, help="Verkauft, wenn der Kurs X% vom absoluten Höchststand seit dem Kauf fällt.")
else:
    TRAILING_STOP_PCT = 999.0 # Im Core deaktiviert
...
if "opt_real" not in st.session_state: st.session_state["opt_real"] = DEFAULT_SHOW_REAL_LIFE 
...
st.sidebar.markdown("---")
show_real_life = st.sidebar.checkbox("💸 Real-Life (Steuern/Gebühren)", key="opt_real")
if show_real_life:
...
if not data.empty:
    ...
    # Neu: Die Momentum-Glättung (berechnet das Momentum auf Basis eines kurzen Durchschnitts)
    smooth_data = data.rolling(window=smooth_days, min_periods=1).mean()
    
    score = (smooth_data.pct_change(21) * w_1m) + \
            (smooth_data.pct_change(63) * w_3m) + \
            (smooth_data.pct_change(126) * w_6m) + \
            (smooth_data.pct_change(252) * w_12m)

    prev_ranks = score.shift(1).rank(axis=1, ascending=False, na_option='bottom')
    is_above_sma = (data.shift(1) > panic_level.shift(1))

    # ... existing code ...
    rebalance_days = data.iloc[::REBAL_INTERVAL].index
    
    daily_pos = pd.DataFrame(0.0, index=data.index, columns=data.columns)
    current_holdings = []
    holding_highs = {} # Gedächtnis für den Trailing Stop
    
    # 🚨 NEU: Tracker für Trailing Stop Treffer (Gedächtnis für das Logbuch)
    ts_hits = pd.DataFrame(False, index=data.index, columns=data.columns)

    for date in data.index:
        ranks = prev_ranks.loc[date]
        above = is_above_sma.loc[date]
        
        # 1. Überprüfung der bestehenden Positionen (SMA & Trailing Stop)
        surviving_holdings = []
        for tkr in current_holdings:
            curr_p = data.loc[date, tkr]
            
            # Höchstkurs aktualisieren
            holding_highs[tkr] = max(holding_highs.get(tkr, curr_p), curr_p)
            
            # Trailing Stop Level berechnen
            ts_level = holding_highs[tkr] * (1 - TRAILING_STOP_PCT / 100.0)
            is_ts_hit = curr_p < ts_level
            
            # ETF überlebt den Tag, wenn SMA-Puffer hält UND Trailing Stop nicht gerissen wurde
            if above[tkr] and not is_ts_hit:
                surviving_holdings.append(tkr)
            else:
                if is_ts_hit:
                    ts_hits.loc[date, tkr] = True  # 🚨 NEU: Wir notieren den TS-Riss für das Logbuch
                if tkr in holding_highs:
                    del holding_highs[tkr] # Aus dem Gedächtnis löschen
                    
        current_holdings = surviving_holdings
     
        # 2. Rebalancing (Rang-Prüfung & Neukäufe)
        if date in rebalance_days:
            # Zuerst: Wer ist aus dem Hold-Rank gefallen?
            keep_after_rank = []
            for tkr in current_holdings:
                if ranks[tkr] <= HOLD_RANK:
                    keep_after_rank.append(tkr)
                else:
                    if tkr in holding_highs:
                        del holding_highs[tkr]
            current_holdings = keep_after_rank
            
            # Dann: Neu kaufen bis das Depot voll ist
            ...
            if needed > 0:
                ...
                for tkr in new_cands[:needed]:
                    current_holdings.append(tkr)
                    holding_highs[tkr] = data.loc[date, tkr] # Höchstkurs-Startpunkt setzen
        
        if current_holdings:
            daily_pos.loc[date, current_holdings] = [1.0 / BUY_RANK] * len(current_holdings)
    ...
    with col_status:
        ...
        if not active_tkrs:
            ...
            for tkr in active_tkrs:
                p = data.loc[last_date, tkr]
                n = panic_level.loc[last_date, tkr]
                h = holding_highs.get(tkr, p)
                ts = h * (1 - TRAILING_STOP_PCT / 100.0)
                
                # Bestimme, welcher Stop aktuell schärfer ist
                actual_stop = max(n, ts)
                stop_type = "Trailing" if ts > n else "SMA"
                
                p_list.append({
                    "Asset": test_tickers.get(tkr, tkr), 
                    "Preis": f"{p:.2f} €", 
                    "Höchstkurs": f"{h:.2f} €",
                    "Stop-Level": f"{actual_stop:.2f} € ({stop_type})", 
                    "Abstand": f"{((p/actual_stop)-1)*100:+.2f}%"
                })
    ...
    with col_dates:
        ...
        days_to_next = REBAL_INTERVAL - days_since
        if days_to_next <= 0:  
            days_to_next = REBAL_INTERVAL
    ...
    if show_screener:
        ...
        st.markdown(f"**📈 Top {HOLD_RANK} ETFs (Halte-Zone): 21-Tage Score-Trend**")
        
        top_tkrs = current_scores.sort_values(ascending=False).index[:HOLD_RANK]
        history_21d = score[top_tkrs].tail(21) * 100 
    ...
    # ==========================================
    # 💸 DAS NEUE REAL-LIFE MODUL (IN-APP) 💸
    # ==========================================
    if show_real_life:
        st.markdown("---")
        st.subheader(f"💸 Real-Life Check: Steuern, Gebühren & Zinseszins")
        st.info("Diese Simulation nutzt exakt die gleichen Rebalancing-Tage wie das Ideal-Modell oben. Kein friktionsloses tägliches Rebalancing, harte Steuerabzüge und Ordergebühren.")
        
        # Optionsbutton für die Grafik
        show_drift_chart = st.checkbox("📈 Portfolio-Drift Grafik anzeigen", value=False)
        ...
        sim_data = data.loc[eval_start_date:]
        
        for i, date in enumerate(sim_data.index):
            ranks = prev_ranks.loc[date]
            above = is_above_sma.loc[date]
            is_rebal = (date in rebalance_days)
            is_first_day = (i == 0) 
            
            # A) Notbremse & Trailing Stop (Täglich geprüft)
            current_tkrs = list(holdings.keys())
            for tkr in current_tkrs:
                curr_p = data.loc[date, tkr]
                
                # Höchstkurs für Real-Life-Modul aktualisieren
                holdings[tkr]['highest_price'] = max(holdings[tkr].get('highest_price', curr_p), curr_p)
                ts_level = holdings[tkr]['highest_price'] * (1 - TRAILING_STOP_PCT / 100.0)
                is_ts_hit = curr_p < ts_level
                
                # Wenn SMA unterschritten ODER Trailing Stop gerissen
                if not above[tkr] or is_ts_hit:
                    shares = holdings[tkr]['shares']
                    buy_price = holdings[tkr]['buy_price']
                    sell_price = curr_p
                    
                    brutto_volumen = shares * sell_price
                    net_val = brutto_volumen - real_fee
                    total_fees += real_fee
                    profit = net_val - (shares * buy_price)
                    tax_paid = 0.0
                    
                    if profit > 0:
                        taxable = max(0, profit - loss_pot)
                        loss_pot = max(0, loss_pot - profit)
                        tax_paid = taxable * real_tax
                    ...
                    # Gesamtdepotwert zum Zeitpunkt des Trades berechnen
                    current_eq = cash + sum(p['shares'] * data.loc[date, t] for t, p in holdings.items())
                    
                    action_str = "🔴 VERKAUF (Trailing Stop)" if is_ts_hit else "🔴 VERKAUF (SMA Notbremse)"
                    
                    trades_log.append({
                        "Datum": date, 
                        "Aktion": action_str, 
                        "Asset": test_tickers.get(tkr, tkr), 
                        "Trade-Volumen": brutto_volumen, 
                        "Gesamtdepot": current_eq,
                        "G/V": profit, 
                        "Steuer": tax_paid, 
                        "Gebühr": real_fee
                    })       
            ...
            # B) Rebalancing
            if is_rebal or is_first_day:
                current_tkrs = list(holdings.keys())
                
                # 1. Rauswurf bei Rank Drop
                if not is_first_day:
                    for tkr in current_tkrs:
                        if ranks[tkr] > HOLD_RANK:
                            shares = holdings[tkr]['shares']
                            buy_price = holdings[tkr]['buy_price']
                            sell_price = data.loc[date, tkr]
                ...
                # 2. Neukauf
                current_tkrs = list(holdings.keys())
                needed = BUY_RANK - len(current_tkrs)
                
                if needed > 0 and cash > (real_fee * 2): 
                    new_cands = [tkr for tkr in ranks.sort_values().index if tkr not in current_tkrs and ranks[tkr] <= BUY_RANK and above[tkr]]
                    cands_to_buy = new_cands[:needed]
                    
                    if len(cands_to_buy) > 0:
                        cash_per_asset = cash / needed
                        for tkr in cands_to_buy:
                            if cash_per_asset > real_fee:
                                investable = cash_per_asset - real_fee
                                buy_price = data.loc[date, tkr]
                                shares = investable / buy_price
                                
                                cash -= cash_per_asset
                                total_fees += real_fee
                                # Initialen Höchstkurs auf den Kaufpreis setzen
                                holdings[tkr] = {'shares': shares, 'buy_price': buy_price, 'highest_price': buy_price}
            ...
            # C) Täglicher Wert & Gewichtungsmessung
            eq = cash
            day_record = {'Date': date, 'Cash': cash}
            for tkr, pos in holdings.items():
                val = pos['shares'] * data.loc[date, tkr]
                eq += val
                day_record[test_tickers.get(tkr, tkr)] = val
            day_record['Real'] = eq
            portfolio_history.append(day_record)
            
        res_df = pd.DataFrame(portfolio_history).set_index('Date').fillna(0)
        
        ideal_scaled = (1 + strat_returns).cumprod() * real_capital
        chart_df_real = pd.DataFrame({
            "🎯 Ideal (Ohne Steuern/Gebühren)": ideal_scaled,
            "💸 Realität (Steuern, Gebühren)": res_df['Real']
        })
        
        st.line_chart(chart_df_real)
        
        real_final = res_df['Real'].iloc[-1]
        real_return_pct = ((real_final / real_capital) - 1) * 100
        
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("Endkapital (Realität)", f"{real_final:,.2f} €")
        col2.metric("Rendite (Realität)", f"{real_return_pct:+.2f} %")
        col3.metric("Verlust durch Steuern", f"- {total_taxes:,.2f} €")
        col4.metric("Verlust durch Ordergebühren", f"- {total_fees:,.2f} €")
        
        if show_drift_chart:
            st.markdown("### ⚖️ Portfolio-Gewichtung & Drift")
            weights_df = res_df.drop(columns=['Real']).div(res_df['Real'], axis=0) * 100
            
            # Chart-Filter: Kleine Positionen bündeln
            max_w = weights_df.max()
            major_assets = max_w[max_w >= 10.0].index.tolist()
            if 'Cash' not in major_assets: major_assets.append('Cash')
            
            minor_assets = [c for c in weights_df.columns if c not in major_assets]
            if minor_assets:
                weights_df['Sonstige'] = weights_df[minor_assets].sum(axis=1)
                weights_df = weights_df.drop(columns=minor_assets)
                
            st.area_chart(weights_df)
        
        # ==========================================
        # 🔥 DIE NEUE TABELLE MIT VOLUMEN UND GESAMTDEPOT
        # ==========================================
        # Füge den heutigen, finalen Stand als letzte Info-Zeile hinzu:
        if trades_log:
            trades_log.append({
                "Datum": data.index[-1], 
                "Aktion": "🏁 HEUTIGER STAND (Unrealisierte Gewinne)", 
                "Asset": "---", 
                "Trade-Volumen": 0.0, 
                "Gesamtdepot": real_final,
                "G/V": 0.0, 
                "Steuer": 0.0, 
                "Gebühr": 0.0
            })
        ...
        if not df_trades.empty:
            st.markdown("---")
            st.write(f"**Verlustverrechnungstopf aktuell:** {loss_pot:,.2f} €")
            df_trades['Datum'] = df_trades['Datum'].dt.strftime('%d.%m.%Y')
        ...
        # ==========================================
        # 🔥 GEWICHTUNGS-DRIFT (ANALYSE)
        # ==========================================
        st.markdown("---")
        st.markdown("### ⚖️ Portfolio-Gewichtung & Drift")
        st.info("Hier siehst du exakt, wie sich die Verteilung im Laufe der Zeit durch Kursentwicklungen verschoben hat. 'Cash' zeigt das nicht investierte Kapital.")
        ...
        with col_w2:
            st.markdown("**📊 Aktuelle Verteilung heute:**")
            curr_w = weights_df.iloc[-1]
            active_w = curr_w[curr_w > 0.01].sort_values(ascending=False) 
            
            for asset, w in active_w.items():
                st.metric(label=asset, value=f"{w:,.1f} %")
    ...
    if show_log:
        ...
        for date in data.loc[twelve_m:].index:
            ...
            if date in rebalance_days or not c_pos.equals(p_pos):
                event_type = "[REBALANCING]" if date in rebalance_days else "[NOTBREMSE / TRAILING STOP]"
                with st.expander(f"📍 {date.strftime('%d.%m.%Y')} - {event_type} - Cash: {(1-c_pos.sum())*100:.0f}%"):
                    ...
                    for r, (tkr, rv) in enumerate(curr_ranks.items(), 1):
                        ...
                        hit_ts = ts_hits.loc[date, tkr] # 🚨 NEU: Abfrage des TS-Gedächtnisses
                        
                        # 🚨 NEU: Präzise Status-Logik
                        if hit_ts:
                            status_icon = "📉 TS Riss"
                        elif not above:
                            status_icon = "❌ SMA Riss"
                        else:
                            status_icon = "✅ Intakt"
                        ...
                        log_rows.append({
                            "Rang": r, 
                            "Asset": test_tickers.get(tkr, tkr), 
                            "Status": status_icon, # 🚨 NEU: Umbenannt von "Trend" zu "Status"
    ...
    if show_manual:
        st.markdown("---")
        if app_mode == "🚀 Satelliten (Offensive)":
            st.header("📘 System-Handbuch: Satelliten-Strategie (Live-Setup 16HT)")
            st.markdown("""
            ### 1. Anlagephilosophie & Zielsetzung
            Das Satelliten-Portfolio bildet die **Offensive** deines Gesamtdepots. Das klare Ziel ist die **Alpha-Generierung** (Überrendite) durch das gezielte Ansteuern von hochvolatilen, trendstarken Sektoren, Ländern und Megatrends. Es handelt sich um ein aggressives Trendfolge-System, das Gewinner reitet und bei Schwäche gnadenlos in Cash geht.

            ### 2. Das Anlage-Universum
            Das System überwacht täglich sorgfältig ausgewählte ETFs aus den Bereichen Technologie (KI, Halbleiter, Cloud), Rohstoffe (Uran, Goldminen), und spezifischen Länder-/Faktor-Indizes.

            ### 3. Die 4 Säulen der Quant-Logik
            * **Säule 1: Der Momentum-Score (Relative Stärke):** Die Rangliste wird täglich berechnet. Der Fast-Twitch Motor fokussiert sich auf aktuelle Ausbrüche (40 % auf 1 Monat) und den soliden Jahrestrend (50 % auf 12 Monate).
            * **Säule 2: SMA 85 & Trailing Stop (Die absoluten Notbremsen):** Fällt der Kurs auf Tagesschlussbasis unter die 85-Tage-Linie ODER fällt er um mehr als X% (z.B. 15%) von seinem Höchststand seit Kauf (Trailing Stop), wird **sofort am nächsten Tag verkauft**, um existenzielle Drawdowns zu vermeiden.
            * **Säule 3: Die weite Hysterese (Steuer-Schutz):** Das System kauft streng die Top 2 (`Buy Rank 2`), erlaubt ihnen aber beim Rebalancing, bis auf Platz 7 abrutschen zu dürfen (`Hysterese 5`), ohne verkauft zu werden. Diese empirisch validierte Einstellung senkt die Transaktionskosten radikal und verschiebt die Realisierung der Abgeltungssteuer massiv nach hinten.
            * **Säule 4: Strikte State-Machine (Der Cooldown):** Fliegt ein ETF durch die Notbremse raus, ist er gesperrt. Er darf nicht sofort wieder gekauft werden, sondern muss bis zum nächsten Rebalancing auf der Strafbank bleiben.
            ...
            * Steht hier ein **VERKAUF**, hat die Notbremse am Vortag (Schlusskurs) ausgelöst. Aktion: Sofort nach Börseneröffnung glattstellen.

            **B. Das 16-tägige Rebalancing (Offensive):**
            * Alle **16 Handelstage** (siehe Countdown) wird das Depot neu gemischt. 
            * Steht hier ein **KAUF**, investierst du vorhandenes Cash in die Top-Assets.
            * Steht hier ein **VERKAUF**, ist ein Asset aus der Halte-Zone gefallen (Platz 8 oder schlechter) und wird ausgetauscht.

            **C. Die 70/30 Soft-Rule (Klumpenrisiko-Management):**
            * Da das System Gewinner laufen lässt, entstehen Ungleichgewichte in der Portfolio-Gewichtung. 
            * **Aktion:** Überprüfe *ausschließlich am 16-tägigen Rebalancing-Tag* das Kuchendiagramm. Hat ein einzelner Satellit ein Gewicht von **über 70 %** erreicht, stutze ihn manuell auf 50 % zurück und schichte das Kapital in den schwächeren ETF um. Dies schützt vor isolierten Sektor-Crashs.
            """)
        else:
